...
def api_v2_request(url, request, required=False, silent=False, die_on_error=True):
    """
    Processes a request on the logentries domain.
    """
    # Obtain response
    ...
    response, conn = get_response(
        "POST", LE_SERVER_API + url, request_encoded,
        silent=silent, die_on_error=die_on_error, domain=Domain.API,
        headers={'Content-Type': 'application/x-www-form-urlencoded'})
...
def cmd_ls_structures():
    ...
    response = api_v2_request('v2/structures', {
        'request': 'list',
        'account': config.user_key,
        'acl': config.user_key,
        }, True)
...
def cmd_ls_patterns(structure_name):
    ...
    response = api_v2_request('v2/structurepatterns', {
        'request': 'list',
        'account': config.user_key,
        'acl': config.user_key,
...
def cmd_add_structure(args):
    ...
    if len(args) > 1:
        ...
        if not pattern.startswith('/'):
            error('Pattern `%s\' does not start with /', pattern)
    ...
    if pattern:
        ...
        # TODO - check that the pattern does not exists already
